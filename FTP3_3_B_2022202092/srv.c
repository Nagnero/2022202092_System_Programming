...
int main(int argc, char **argv)
{
    ...
    while (1)
    {
        ...
        read(client_fd, ip_port_str, sizeof(ip_port_str)); // Read data from client
        ...
        { // Read lines from file
            ...
            while (pattern != NULL && ip_segment != NULL)
            { // Check each segment of IP
        ...
        {
            write(STDOUT_FILENO, "REJECTION\n", strlen("REJECTION\n"));
            write(client_fd, "REJECTION", strlen("REJECTION"));
            close(client_fd);
            continue;
        }
...
void sh_chld(int signum)
{
    ...
    if (exiting)
        return; // ignore process terminated by SIGTERM or during exiting
    pid_t pid;
    int status;
...
// log_file                                                         //
// =================================================================//
// Input: msg -> messages written in log file                       //
//                                                                  //
// Output: None                                                     //
//                                                                  //
// Purpose: Write server messages into logfile                      //
//////////////////////////////////////////////////////////////////////
void log_file(char *msg, int a)
{
    FILE *fp;
    ...
    if (a == 1)
    {
        sprintf(log, "%s %s", buf, msg);
    }
    else
    {
        // time, ip address, port number, username, msg
        sprintf(log, "%s %s [%s:%d] %s %s", buf, inet_ntoa(client_addr.sin_addr), client_addr.sin_port, username, msg);
    }
    fputs(log, fp);
    fclose(fp);
}
